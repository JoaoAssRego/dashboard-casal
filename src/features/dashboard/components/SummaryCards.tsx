...
export function SummaryCards({ balance = 0, income = 0, expense = 0, isLoading = false, minimal = false }: SummaryCardsProps) {
  ...
  return (
    <div className="grid gap-4 md:grid-cols-3">
      {/* Saldo Atual - Botão Interativo no modo Minimal */}
      <Card 
        onClick={() => minimal && navigate('/balance')}
        className={cn(
          "border-purple-500/30 shadow-xl shadow-purple-500/10 bg-gradient-to-br from-indigo-950 via-purple-900/40 to-slate-950 overflow-hidden relative",
          minimal && "cursor-pointer hover:ring-2 ring-purple-500/50 transition-all active:scale-[0.98]"
        )}
      >
        ...
        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2 relative z-10">
          ...
          {minimal ? <ChevronRight className="h-5 w-5 text-purple-400" /> : <Wallet className="h-5 w-5 text-purple-400" />}
        </CardHeader>
